...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        fc1_val, fc1_cache = affine_forward(flat_X, self.params['W1'], self.params['b1'])

        relu1_val, relu1_cache = relu_forward(fc1_val)
        
        fc2_val, fc2_cache = affine_forward(relu1_val, self.params['W2'], self.params['b2'])

        scores = np.copy(fc2_val)
        ...
        reg_loss1 = np.sum(np.square(self.params['W1']))
        reg_loss2 = np.sum(np.square(self.params['W2']))

        loss += 0.5*self.reg*(reg_loss1 + reg_loss2)
